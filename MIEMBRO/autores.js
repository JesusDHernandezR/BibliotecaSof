...
function consultaDB(tx) {
  //Crear la tabla contactos si no esta creada
	...
	tx.executeSql('select autor, count(autor) from Libro group by autor having count(autor)>0',[],(tx, results) => {
		var lista = [];
		for(var i =0; i < results.rows.length; i++){
            lista.push('<tr><th><i>'+results.rows.item(i).autor+'</i></th></tr>');
		}
		document.getElementById('resultados').innerHTML = lista.join('');
	});
}

//SELECT isbn, titulo, facultad, autor,resumen FROM Libro


function consultaAutoresDB(tx) {
  //Crear la tabla contactos si no esta creada
	tx.executeSql('CREATE TABLE IF NOT EXISTS Libro (isbn, titulo, facultad, autor,resumen)');
	tx.executeSql('SELECT isbn, titulo, facultad, autor,resumen FROM Libro WHERE autor="'+autor+'"',[],(tx, results) => {
		var lista = [];
		for(var i =0; i < results.rows.length; i++){
		...
		document.getElementById('resultadosxautores').innerHTML = lista.join('');
	});
}

function consultaAutores(){
	autor=document.getElementById("Bautor").value;
	db.transaction(consultaAutoresDB);
}
